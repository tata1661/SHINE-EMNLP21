...
class Trainer(object):
    def __init__(self, params):
        ...
        self.model = SHINE(self.adj_dict, self.features_dict, self.in_features_dim, self.out_features_dim, self.train_idx, self.test_idx, self.params)
    ...
    def load_data(self):
        ...
        feature_dict['word_emb'] = pkl.load(open(self.data_path + './word_emb.pkl', 'rb'))
    ...
    def save(self, path=None):
        if not path:
            path = self.save_path
        paddle.save(self.model.state_dict(), path + './{}/save_model_new'.format(self.dataset_name))

    def load(self):
        self.model.load_state_dict(paddle.load(self.save_path))
